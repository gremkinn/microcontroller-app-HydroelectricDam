...
extern int openLED_status;
extern int closeLED_status;
...
void __attribute__((picinterrupt(("")))) isr(void) {
    ...
    if (PIR0bits.INTF == 1 && INTPPS == 0x09) {
        PIR0bits.INTF = 0;
        INTPPS = 0x08;
        sevOff();
        emergency_LED();
    ...
    if (PIR4bits.TMR1IF == 1) {
        ...
        if (PORTBbits.RB2 == 1 && power_count == 2) {
            LCD_setPos(2, 5);
            msgLCD("+");
            powerLvl++;
            power_count = 0;
        ...
        } else if (PORTBbits.RB2 == 0 && power_count == 4) {
            LCD_setPos(2, 5);
            msgLCD("-");
            powerLvl++;
            power_count = 0;
    ...
    if (PIR4bits.TMR5IF == 1) {
        ...
        if (openLED_status == 1) {
            TMR3H = 0x0B;
            TMR3L = 0xDC;
            PORTAbits.RA3 = ~PORTAbits.RA3, PORTAbits.RA2 = 0;
            PORTAbits.RA1 = ~PORTAbits.RA1;
        } else if (closeLED_status == 1) {
            TMR3H = 0x0B;
            TMR3L = 0xDC;
            PORTAbits.RA3 = ~PORTAbits.RA3, PORTAbits.RA2 = ~PORTAbits.RA2, PORTAbits.RA1 = ~PORTAbits.RA1;
        }
    }
}
